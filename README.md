...
我们的设计理念是最大程度的减少AI到论文的诸多不必要的体力活，但绝不是完全AI替代作者。
...
AI 会照着 [INSTALL.md](INSTALL.md)(一份可执行的操作手册)把五个技能接进你的 AI 工具(Claude Code / Codex / OpenCode / Hermes Agent 都行)、装 pandoc 工具链、注册保护钩子(Claude Code 原生钩子；Codex / OpenCode 用兼容适配层),**你一条命令都不用自己敲**。想手动装,INSTALL.md 里也逐条列了命令。

### 环境要求

- **Windows + Microsoft Word** —— 摄取(`md-unpack`)靠 Word COM 读引用域/图。*(macOS 暂不支持;摄取之后全跨平台。)*
- **Python 3** + **PowerShell**(Windows 自带 5.1 即可)。
- **AI 编程工具** —— 为 **Claude Code** 深度打造，同时兼容 **Codex / OpenCode / Hermes Agent** 等 Agent Skills 工具。Claude 使用成熟原生钩子，Codex / OpenCode 可安装兼容保护层；所有环境仍由脚本内置闸门 + [AGENTS.md](AGENTS.md) 承担第一层安全，详见 [INSTALL.md](INSTALL.md)。
...
Our design philosophy is to minimize, to the greatest extent, the many unnecessary manual chores between AI and a finished paper - but never to have AI replace the author.
The md-paper pipeline is long - ingest, organize, draft, audit, compile - and every stage has a **human gate**: comments land only after confirmation, edits pass only after review, citation changes need authorization. Walk the whole pipeline and the manuscript is still **fundamentally the author's own work**. What AI removes is the typing, pasting, format-fixing, and issuing commands one by one; **every place that needs the author's thinking is still there**: how to read the reviewer comments, how far to go, which citations to keep or cut, whether the final draft is final - all the user's own decisions.

Our stance: **AI is only an aid - it makes revision more convenient, but it cannot replace the author's own thinking and originality.** The tool runs errands; it does not think for the author.
...
The AI follows [INSTALL.md](INSTALL.md) — an executable runbook — to link the five skills into your AI tool, install the pandoc toolchain, and register protection hooks (native Claude hooks plus Codex/OpenCode adapters). **You don't type a single command yourself.** Prefer manual? INSTALL.md lists every command.
...
- **AI coding tool** — built first for **Claude Code** and compatible with **Codex / OpenCode / Hermes Agent**. Claude uses its native hooks; Codex and OpenCode can install compatible adapters. Script gates plus [AGENTS.md](AGENTS.md) remain the authoritative first layer everywhere; see [INSTALL.md](INSTALL.md).

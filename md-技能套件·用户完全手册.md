...
> **第三代 Markdown 真源学术写作工具链——从 Word 原稿到带活 Zotero 引用的新稿，全程 AI 辅助、纯文本安全、pandoc 编译。**
>
> 本手册写给**作者（用户）**：不用懂代码，读完就能独立完成"原稿摄取 → AI 批量改稿 → 出带活 Zotero 域的 Word"全流程。
> 开发者向的技术细节见 `md-技能套件·开发手册.md`。
>
> **建立**：2026-06-17 · **版本**：v1.0
...
> 2. **第一次用，必须先确认两个"防护钩子"真生效了，再开始改稿。** 这俩钩子是防止 AI 改稿时把你的引用/正文改丢的命脉闸——没装上 = 没保护，而且**不报警**。装法见《新电脑设置指南》「md-* 套件防呆硬闸 Hook」一节，装完跑它的「验证」命令，要看到 `deny` 才算数；看不到 = 钩子没生效，**先别改稿**，照该节排查（junction 在不在 / settings.json 注册没 / 重启 Claude Code 没 / `.ps1` 是不是 UTF-8 带 BOM）。**⚠️ "重启 Claude Code" 这步特别容易做错**：在 VS Code 里要**新开一个 Claude 对话**，不是关掉面板再打开（重开若恢复了旧会话、钩子还是没加载）；`/clear` 对钩子不可靠、别用它充重启。详见该节「重启 Claude Code 到底怎么操作」。
...
>
> 现阶段最稳的是**英文、纯文字为主、用 Word 原生公式（或无公式）、引用用 Zotero** 的社科稿。带上述特征的稿子等后续版本再试——出了问题不是你操作错，是工具还没覆盖到，别为难自己也别急着找作者。

---

# 第一部分：核心优势——为什么选 md-*？

> 这一节也是你发朋友圈/学术群安利的素材。每一条都是真机实测跑通、不是"设计目标"。
...
> ✅ md-* 方案：**Markdown 写作 + pandoc 编译出 Word**。你在纯文本里写，最终一键生成格式完美的 .docx。不需要学 LaTeX，不需要手搓 Word COM。
...
| 传统改稿（手搓 Word） | md-* 第三代（Markdown 真源 + AI + pandoc） |
| --- | --- |
...
> ⚡ 对比第二代 docx-*：原地编辑 docx 时要小心翼翼地保住占位符和活域（最脆一环，实测通过率仅 ~36%）。md-* 是**生成时一次性建活域**，每次都是新建、无所谓"保"。
...
这是第二代 docx-swarm 的精华、第三代原样继承并大幅简化：
...
| 你要做什么 | 用哪套 |
|------------|--------|
| 在既有期刊稿上做外科小改（保留修订痕迹） | 第二代 docx-* |
| 整篇论文大重构（重写理论框架、重写实证、新增大量图表） | **第三代 md-*** |
| 从零写新论文、要活 Zotero 引用 | **第三代 md-*** |
| 审稿意见回复（生成 Response Letter） | 第一代 reviewer-* |
...
flowchart TD
    ...
    B -->|"无"| C["内部调 docx-unpack<br/>Word COM 拆稿<br/>抽取 Zotero 域代码/图表/脚注"]
    B -->|"有"| D["复用已有 manifest"]
...
| 我要的是修订模式、在既有 docx 上改 | 这是第二代 `docx-*` 的活 | 用 `/docx-unpack → /docx-add → /docx-commit`，不是 md-* |
...
| 代 | 组织主线 | 真源 | 出稿方式 | 修订痕迹 | 适用场景 |
|---|---------|------|---------|:---:|---------|
| **一代** `reviewer-*` | 审稿意见回复七步流水线 | 原稿 docx | COM 写回 | 要 | 审稿意见全流程管理+Response Letter |
| **二代** `docx-*` | Git 式版本控制的 docx 编辑 | 原稿 docx | COM 写回（Tracked Changes） | 要 | 既有期刊稿的外科小改 |
| **三代** `md-*` | Markdown 真源 → pandoc 编译 | **manuscript.md**（纯文本） | pandoc 全新生成 | 不要 | 大重构/新写 + 活 Zotero 引用 |

**三代不互斥、按场景选**：
- 发过一轮被要求小修 → 二代 `docx-*`（保修订痕迹）
- 要彻底重构理论框架和实证 → 三代 `md-*`（纯文本安全、大段重写不崩）
- 从头新写论文 → 三代 `md-*`（Markdown 写作 + 最终编译出带活引用的 Word）
- 管理审稿意见+出 Response Letter → 一代 `reviewer-*`

---

> **本手册持续更新。** 遇到新问题、踩了新坑、发现新的最佳实践——更新本文档，让下一个用户（包括未来的你）受益。
>
> 技术细节/开发进度/原型验证数据 → 见 `md-技能套件·开发手册.md`。
>
> 新电脑设置（Junction 一键建、hook 注册等）→ 见 `新电脑设置指南.md`。
